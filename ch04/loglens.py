...
def parse_jsonl_line(line):
    """JSON Lines の1行から (レベル, 時刻) を取り出す。

    レベルが3種以外・キー欠落・JSON として不正な行は (OTHER, None) を返す。
    """
    try:
    ...
    with open(path, encoding="utf-8") as f:
        for line in f:
            if not line.strip():
                continue
            if is_jsonl:
                level, time = parse_jsonl_line(line)
            else:
                level, time = parse_level(line), parse_time(line)
...
def cmd_count(args):
    ...
    try:
        for logfile in args.logfile:
            counts += count_levels(logfile, since=args.since, until=args.until)
    except OSError as e:
