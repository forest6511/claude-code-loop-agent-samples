# loglens

ログファイルのレベル別件数を集計する CLI。

## 使い方

    python3 loglens.py count app.log
    python3 loglens.py count app.log --level ERROR
    python3 loglens.py count app.log --since 09:00:00 --until 12:00:00
    python3 loglens.py count app.jsonl
    python3 loglens.py count a.log b.jsonl
    python3 loglens.py count app.log --top 3
    python3 loglens.py count app.log --format json
    python3 loglens.py count app.log --format csv
    python3 loglens.py count app.log --regex "timeout|refused"
    python3 loglens.py count app.log --stats
    python3 loglens.py count app.log --output result.txt

`--level` は ERROR / WARN / INFO / OTHER のいずれかを指定します。
`--since` / `--until` は HH:MM:SS 形式で集計対象の時刻範囲を指定します。
`--top N` は頻出メッセージの上位 N 件を、レベル別集計の後に
「件数 メッセージ」の形式で1行1件表示します。同数のメッセージは
ログで先に出現した順に並び、OTHER に分類される行は集計対象に
含めません。`--level` / `--since` / `--until` / `.jsonl` 入力 /
複数ファイルと併用でき、絞り込み後の行を集計対象とします。

`sample.log` で `--top 3` を実行した例:

    $ python3 loglens.py count sample.log --top 3
    ERROR 3
    WARN  4
    INFO  11
    OTHER 2
    TOTAL 20
    1 Server started on port 8080
    1 User login succeeded user_id=1023
    1 GET /api/items 200 12ms

`--format` は text / json / csv のいずれかを指定します（既定は text で、
出力は従来と同じです）。`--format json` は集計結果をインデント2の
JSON 1オブジェクトで出力します。`--top N` 併用時は `top` キーが
最後に追加され、`--level` 指定時は `counts` にそのレベルだけが
入ります。他のオプション・`.jsonl` 入力・複数ファイルとも併用でき、
絞り込み・合算後の集計結果を出力します。

`sample.log` で `--format json --top 1` を実行した例:

    $ python3 loglens.py count sample.log --format json --top 1
    {
      "counts": {
        "ERROR": 3,
        "WARN": 4,
        "INFO": 11,
        "OTHER": 2
      },
      "total": 20,
      "top": [
        {
          "count": 1,
          "message": "Server started on port 8080"
        }
      ]
    }

`--format csv` は1行目にヘッダ `level,count`、以降に ERROR / WARN /
INFO / OTHER / TOTAL の順で「レベル,件数」を出力します。`--top N`
併用時は空行を1行おき、ヘッダ `count,message` に続けて上位順に
「件数,メッセージ」を出力します。メッセージにカンマや引用符が
含まれる場合は標準の CSV 引用ルール（ダブルクォート囲み）で
守ります。`--level` 指定時はそのレベルの1行だけを出力します。
他のオプション・`.jsonl` 入力・複数ファイルとも併用できます。

`sample.log` で `--format csv --top 3` を実行した例:

    $ python3 loglens.py count sample.log --format csv --top 3
    level,count
    ERROR,3
    WARN,4
    INFO,11
    OTHER,2
    TOTAL,20

    count,message
    1,Server started on port 8080
    1,User login succeeded user_id=1023
    1,GET /api/items 200 12ms

`--regex PATTERN` は、メッセージ部分がパターンに一致
（Python の正規表現・`re.search` の部分一致・大文字小文字は区別）する
行だけを集計対象にします。メッセージを持たない OTHER 行は集計に
含めません。不正なパターンを指定した場合はパターンと理由を標準
エラーに出して exit 2 で終了します。`--level` / `--since` / `--until` /
`--top` / `--format json` / `--format csv` / `.jsonl` 入力 /
複数ファイルと併用でき、すべての絞り込みの積で集計します。

`sample.log` で `--regex "timeout|refused"` を実行した例:

    $ python3 loglens.py count sample.log --regex "timeout|refused"
    ERROR 1
    WARN  0
    INFO  0
    OTHER 0
    TOTAL 1

`--stats` は、通常の集計（`--top` 併用時はその後）に空行を1行おいて、
行が1件以上ある時間帯だけを「HH時 N件」の形式で時刻順に表示します
（HH は 00〜23 の2桁）。対象は `--level` / `--since` / `--until` /
`--regex` で絞り込んだ後の行で、タイムスタンプを持たない行（OTHER 等）
は含めません。`--format json` 併用時は `stats` キー（`{"HH": 件数}`
形式・出現した時間帯のみ・時刻順）が最後に追加されます。
`--format csv` とは併用できず、error を標準エラーに出して exit 2 で
終了します。

`sample.log` で `--stats` を実行した例:

    $ python3 loglens.py count sample.log --stats
    ERROR 3
    WARN  4
    INFO  11
    OTHER 2
    TOTAL 20

    09時 18件

`--output PATH` は、標準出力に出すはずの集計結果（`--format` text /
json / csv すべて）をそのまま PATH のファイルへ UTF-8 で書き出し、
標準出力には `wrote: PATH` を1行だけ表示します。既存ファイルは
上書きします。PATH の親ディレクトリが存在しない場合は理由を標準
エラーに出して exit 2 で終了します。`--output` を指定しないときの
出力は従来と同じです。

`sample.log` で `--output` を実行した例:

    $ python3 loglens.py count sample.log --output result.txt
    wrote: result.txt
    $ cat result.txt
    ERROR 3
    WARN  4
    INFO  11
    OTHER 2
    TOTAL 20

ファイルは複数指定でき、全ファイルの件数を合算して1つの集計として
表示します（ファイル別の内訳は表示しません）。`.log` と `.jsonl` の
混在も可能で、形式はファイルごとに判定します。`--level` / `--since` /
`--until` はすべてのファイルに適用されます。

## alert サブコマンド

    python3 loglens.py alert app.log --threshold 10
    python3 loglens.py alert app.log --threshold 10 --level WARN
    python3 loglens.py alert a.log b.jsonl --threshold 5 --regex timeout

指定レベル（`--level`・未指定時は ERROR）の件数がしきい値
`--threshold N`（必須・1 以上の整数）**以上**なら「発火」として
exit 3 で終了します。件数は count と同じロジックで数え、複数
ファイルは合算し、`--since` / `--until` / `--regex` で絞り込んだ
後の件数で判定します。

発火の有無にかかわらず、標準出力に1行サマリを出します:

    $ python3 loglens.py alert sample.log --threshold 4
    OK: ERROR 3 < 4        （非発火・exit 0）
    $ python3 loglens.py alert sample.log --threshold 2
    ALERT: ERROR 3 >= 2    （発火・exit 3）

発火時は同じ行を標準エラーにも出します（標準出力がパイプで
消費されても気づけるようにするため）。

`--level OTHER` と `--regex`、`--level OTHER` と `--since` /
`--until` の併用は、対象行が常に除外され永遠に発火しない設定に
なるため exit 2 で拒否します。`--threshold` の未指定・非整数・
0 以下も exit 2 です。

## 終了コード

- 0: 正常終了（alert では非発火）
- 1: ファイルが読めない（存在しない・権限がない等。理由を標準エラーに表示）
- 2: `--regex` のパターンが不正、`--stats` と `--format csv` の併用、
  `--output` の書き出し先に書き込めない（親ディレクトリがない等）、
  または alert の引数エラー（`--threshold` 未指定・非整数・0 以下、
  `--level OTHER` とフィルタの併用。いずれも理由を標準エラーに表示）
- 3: alert の発火（指定レベルの件数がしきい値以上）

## 入力形式

拡張子が `.jsonl` のファイルは JSON Lines（1行 = 1 JSON オブジェクト）として
解釈します。各行のキーは `timestamp`（`2026-07-18 09:12:01` 形式）、
`level`（ERROR / WARN / INFO）、`message`（文字列）です。
レベルが3種以外・キー欠落・JSON として不正な行は OTHER に数えます。
空行はテキスト形式と同じくスキップします。
`--level` / `--since` / `--until` はテキスト形式と同じ挙動で併用できます。

## CI

PR を開くと GitHub Actions で検証ループ (verify_loop.py) が走り、
pytest・ruff・カバレッジ90%の3点ゲートで合否を判定します。
